...
func NewBlockUnlocker(cfg *UnlockerConfig, backend *storage.RedisClient) *BlockUnlocker {
	if len(cfg.PoolFeeAddress) != 0 && !IsValidHexAddress(cfg.PoolFeeAddress) {
...
func (u *BlockUnlocker) unlockCandidates(candidates []*storage.BlockData) (*UnlockResult, error) {
	...
	for _, candidate := range candidates {
		...
		for i := int64(minDepth * -1); i < minDepth; i++ {
			...
			if err != nil {
				Error.Printf("Error while retrieving block %v from node: %v", height, err)
				return nil, err
			}
			if block == nil {
			...
			for uncleIndex, uncleHash := range block.Uncles {
				...
				if matchCandidate(uncle, candidate) {
					...
					result.maturedBlocks = append(result.maturedBlocks, candidate)
					Info.Printf("Mature uncle %v/%v of reward %v with hash: %v", candidate.Height, candidate.UncleHeight,
						FormatReward(candidate.Reward), uncle.Hash[0:10])
					break
				}
			}
...
func sendSyslog(to string, logMsg string) error {
...
func (u *BlockUnlocker) unlockPendingBlocks() {
	if u.halt {
		if u.config.Syslog.Enabled {
			err := sendSyslog(u.config.Syslog.To, u.lastErrMsg)
			if err != nil {
				Error.Printf("sendSyslog [%s] to [%s] error", u.lastErrMsg, u.config.Syslog.To)
			}
		}
	...
	if err != nil {
		u.halt = true
		u.lastFail = err
		u.lastErrMsg = fmt.Sprintf("Unable to get current blockchain height from node: %v", err)
		Error.Printf(u.lastErrMsg)
		return
	}
	...
	if err != nil {
		u.halt = true
		u.lastFail = err
		u.lastErrMsg = fmt.Sprintf("Can't parse pending block number: %v", err)
		Error.Printf(u.lastErrMsg)
		return
	}
	...
	if err != nil {
		u.halt = true
		u.lastFail = err
		u.lastErrMsg = fmt.Sprintf("Failed to get block candidates from backend: %v", err)
		Error.Printf(u.lastErrMsg)
		return
	}
	...
	if err != nil {
		u.halt = true
		u.lastFail = err
		u.lastErrMsg = fmt.Sprintf("Failed to unlock blocks: %v", err)
		Error.Printf(u.lastErrMsg)
		return
	}
	...
	if err != nil {
		u.halt = true
		u.lastFail = err
		u.lastErrMsg = fmt.Sprintf("Failed to insert orphaned blocks into backend: %v", err)
		Error.Printf(u.lastErrMsg)
		return
	} else {
	...
	for _, block := range result.maturedBlocks {
		...
		if err != nil {
			u.halt = true
			u.lastFail = err
			u.lastErrMsg = fmt.Sprintf("Failed to calculate rewards for round %v: %v", block.RoundKey(), err)
			Error.Printf(u.lastErrMsg)
			return
		}
		...
		if err != nil {
			u.halt = true
			u.lastFail = err
			u.lastErrMsg = fmt.Sprintf("Failed to credit rewards for round %v: %v", block.RoundKey(), err)
			Error.Printf(u.lastErrMsg)
			return
		}
...
func (u *BlockUnlocker) unlockAndCreditMiners() {
	...
	for _, block := range result.orphanedBlocks {
		...
		if err != nil {
			u.halt = true
			u.lastFail = err
			u.lastErrMsg = fmt.Sprintf("Failed to insert orphaned block into backend: %v", err)
			Error.Printf(u.lastErrMsg)
			return
		}
	}
...
func getConstReward(height int64) *big.Int {
	if height >= istanbulHardForkHeight {
		return new(big.Int).Set(istanbulReward)
